...
export default function ServicesPage() {
  ...
  // Works with HomePage: navigate(`/services?s=${service.slug}`)
  useEffect(() => {
    if (!sectionId) return;
          ...
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.slug}
                id={service.slug}
                className={`scroll-mt-28 mb-12 sm:mb-16 md:mb-20 last:mb-0 ${index !== 0 ? 'pt-10 sm:pt-12 md:pt-16 border-t border-gray-200' : ''
                  }`}
              >
                <div
                  className={`grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-start ${isEven ? '' : 'lg:flex-row-reverse'
                    }`}
                >
                  {/* Image */}
